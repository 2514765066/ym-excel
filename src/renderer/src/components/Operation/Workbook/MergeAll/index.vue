<template>
  <Base
    :title="titleMap.workbook.mergeAll.title"
    :icon="titleMap.workbook.mergeAll.icon"
    @remove="emits('remove')"
    :open="rule.length != 0"
  >
    <template #after>
    ...
    <Drag v-model="rule" group="mergeAll" class="gap-1.5">
      <Item
...
import { MergeAll } from "@type/operation";
...
const rule = defineModel<MergeAll[]>({ required: true });

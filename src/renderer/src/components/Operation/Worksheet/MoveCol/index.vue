<template>
  <Base
    :title="titleMap.worksheet.moveCol.title"
    :icon="titleMap.worksheet.moveCol.icon"
    @remove="emits('remove')"
    :open="rule.length != 0"
  >
    <template #after>
    ...
    <Drag v-model="rule" group="moveCol" class="gap-1.5">
      <Item
        v-for="(_, i) of rule"
        :key="nanoid()"
        v-model="rule[i]"
...
import { MoveCol } from "@type/operation";
...
const rule = defineModel<MoveCol[]>({
  required: true,
});

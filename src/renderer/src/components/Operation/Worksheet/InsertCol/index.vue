<template>
  <Base
    :title="titleMap.worksheet.insertCol.title"
    :icon="titleMap.worksheet.insertCol.icon"
    @remove="emits('remove')"
    :open="rule.length != 0"
  >
    <template #after>
    ...
    <Drag v-model="rule" group="insertCol" class="gap-1.5">
      <Item
        v-for="(_, i) of rule"
        :key="nanoid()"
        v-model="rule[i]"
...
import { InsertCol } from "@type/operation";
...
const rule = defineModel<InsertCol[]>({
  required: true,
});

<template>
  <Base
    :title="titleMap.worksheet.clearCol.title"
    :icon="titleMap.worksheet.clearCol.icon"
    @remove="emits('remove')"
    :open="rule.length != 0"
  >
    <template #after>
    ...
    <Drag v-model="rule" group="clearCol" class="gap-1.5">
      <Item
        v-for="(_, i) of rule"
        :key="nanoid()"
        v-model="rule[i]"
...
import { ClearCol } from "@type/operation";
...
const rule = defineModel<ClearCol[]>({
  required: true,
});

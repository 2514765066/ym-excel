<template>
  <Base
    :title="titleMap.worksheet.compute.title"
    :icon="titleMap.worksheet.compute.icon"
    @remove="emits('remove')"
    :open="rule.length != 0"
  >
    <template #after>
    ...
    <Drag v-model="rule" group="compute" class="gap-1.5">
      <Item
...
const rule = defineModel<Compute[]>({ required: true });

<template>
  <Base
    :title="titleMap.worksheet.moveRow.title"
    :icon="titleMap.worksheet.moveRow.icon"
    @remove="emits('remove')"
    :open="rule.length != 0"
  >
    <template #after>
    ...
    <Drag v-model="rule" group="moveRow" class="gap-1.5">
      <Item
        v-for="(_, i) of rule"
        :key="nanoid()"
        v-model="rule[i]"
...
import { MoveRow } from "@type/operation";
...
const rule = defineModel<MoveRow[]>({ required: true });

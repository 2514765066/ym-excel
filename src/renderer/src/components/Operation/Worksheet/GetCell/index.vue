<template>
  <Base
    :title="titleMap.worksheet.getCell.title"
    :icon="titleMap.worksheet.getCell.icon"
    @remove="emits('remove')"
    :open="rule.length != 0"
  >
    <template #after>
    ...
    <Drag v-model="rule" group="getCell" class="gap-1.5">
      <Item
...
import { GetCell } from "@type/operation";
...
const rule = defineModel<GetCell[]>({ required: true });

<template>
  <Base
    :title="titleMap.worksheet.insertRow.title"
    :icon="titleMap.worksheet.insertRow.icon"
    @remove="emits('remove')"
    :open="rule.length != 0"
  >
    <template #after>
    ...
    <Drag v-model="rule" group="insertRow" class="gap-1.5">
      <Item
        v-for="(_, i) of rule"
        :key="nanoid()"
        v-model="rule[i]"
...
import { InsertRow } from "@type/operation";
...
const rule = defineModel<InsertRow[]>({
  required: true,
});

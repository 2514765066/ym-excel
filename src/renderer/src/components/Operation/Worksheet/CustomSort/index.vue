<template>
  <Base
    :title="titleMap.worksheet.customSort.title"
    :icon="titleMap.worksheet.customSort.icon"
    @remove="emits('remove')"
    :open="rule.length != 0"
  >
    <template #after>
    ...
    <Drag v-model="rule" group="customSort" class="gap-1.5">
      <Item
        v-for="(_, i) of rule"
        :key="nanoid()"
        v-model="rule[i]"
...
import { CustomSort } from "@type/operation";
...
const rule = defineModel<CustomSort[]>({
  required: true,
});

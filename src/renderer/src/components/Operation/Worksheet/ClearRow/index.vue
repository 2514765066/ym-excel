<template>
  <Base
    :title="titleMap.worksheet.clearRow.title"
    :icon="titleMap.worksheet.clearRow.icon"
    @remove="emits('remove')"
    :open="rule.length != 0"
  >
    <template #after>
    ...
    <Drag v-model="rule" group="clearRow" class="gap-1.5">
      <Item
        v-for="(_, i) of rule"
        :key="nanoid()"
        v-model="rule[i]"
...
import { ClearRow } from "@type/operation";
...
const rule = defineModel<ClearRow[]>({
  required: true,
});
